...
- One connection-level idle clock is reset by a completed forward in **either** direction. Sustained upload-only or download-only traffic therefore stays open while the other direction is quiet. A connection with no completed forwarding closes after the configured idle interval; an individual stalled socket send is also bounded by that interval. Once settled `actual_bytes` reaches quota, the connection closes immediately, without waiting for another payload or idle timeout. A balance held by another pending permit is allowed to settle first.
