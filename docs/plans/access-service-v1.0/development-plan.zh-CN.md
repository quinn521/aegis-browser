...
| Q：W0 新候选回归 | 0197–0199 增加 relay 取消/watchdog、真实缓存命中后 REJECT、已提交 MHTML 子帧预取早拒绝；精确身份和本轮结果见 QUALITY-HANDOFF 与独立原始回执 | 候选须重新执行 source admission、原生与真实入口矩阵及质量门；旧 H 的通过不转作新 H 的结果，完整 BFCache/prerender、G0 与主行仍分别判断 |
