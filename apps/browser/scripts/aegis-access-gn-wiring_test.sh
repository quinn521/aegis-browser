...
DOWNLOAD_PATCH_FILE="$BROWSER_DIR/patches/0148-feat-aegis-gate-frame-download-traffic.patch"
...
rg -Fq 'URLLoaderFactoryType::kDownload && frame' \
  "$DOWNLOAD_PATCH_FILE" ||
  fail "patch 0148 must wire only frame-backed download factories"
rg -Fq 'MaybeProxyDownload' \
  "$DOWNLOAD_PATCH_FILE" ||
  fail "patch 0148 must install the frame-backed download Access wrapper"
rg -Fq 'MaybeProxyFrameOwnedFactory' \
  "$DOWNLOAD_PATCH_FILE" ||
  fail "patch 0148 must reuse trusted frame-owned attribution"
rg -Fq 'FrameBackedDownloadWithoutPolicyPreservesNativePath' \
  "$DOWNLOAD_PATCH_FILE" ||
  fail "patch 0148 must preserve native frame-backed downloads without policy"
rg -Fq 'FrameBackedDownloadUsesSelectedProxy' \
  "$DOWNLOAD_PATCH_FILE" ||
  fail "patch 0148 must browser-test selected frame-backed download routing"
rg -Fq 'FrameBackedDownloadWithoutEndpointFailsClosed' \
  "$DOWNLOAD_PATCH_FILE" ||
  fail "patch 0148 must fail closed when download PROXY lacks an endpoint"
if rg -Fq 'URLLoaderFactoryType::kDownload && !frame' "$DOWNLOAD_PATCH_FILE"; then
  fail "patch 0148 must not claim frame-less/background downloads"
fi
...
[[ "$(rg -F -c '0148-feat-aegis-gate-frame-download-traffic.patch' \
...
0148-feat-aegis-gate-frame-download-traffic.patch
EOF
)"
