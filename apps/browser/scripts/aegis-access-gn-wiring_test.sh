...
BLOCK_TERMINATION_PATCH_FILE="$BROWSER_DIR/patches/0136-feat-aegis-terminate-inflight-block-requests.patch"
...
rg -Fq 'AccessBlockOperationResult AccessRequestDispatchState::BeginBlockOperation' \
  "$BLOCK_TERMINATION_PATCH_FILE" ||
  fail "patch 0136 must expose the Profile-owned BLOCK operation coordinator"
rg -Fq 'barriers_.InstallBlockBarrier(std::move(barrier));' \
  "$BLOCK_TERMINATION_PATCH_FILE" ||
  fail "patch 0136 must install the BLOCK barrier before cancellation"
rg -Fq 'ownership_.CancelMatchingPageTarget(selector);' \
  "$BLOCK_TERMINATION_PATCH_FILE" ||
  fail "patch 0136 must cancel only the trusted matching page target"
rg -Fq 'ReleaseBlockOperation' "$BLOCK_TERMINATION_PATCH_FILE" ||
  fail "patch 0136 must require explicit BLOCK barrier release"
rg -Fq 'CreateTerminationHandle' "$BLOCK_TERMINATION_PATCH_FILE" ||
  fail "patch 0136 must keep a real termination handle on tracked requests"
rg -Fq 'AccessProxyingURLTrackedRequest' "$BLOCK_TERMINATION_PATCH_FILE" ||
  fail "patch 0136 must isolate tracked request lifecycle ownership"
...
[[ "$(tail -n 3 "$SERIES_FILE" | head -n 1)" == \
  ...
  fail "patch 0134 must immediately precede patch 0135"
[[ "$(tail -n 2 "$SERIES_FILE" | head -n 1)" == \
  "0135-feat-aegis-gate-real-document-url-loader-traffic.patch" ]] ||
  fail "patch 0135 must immediately precede patch 0136"
[[ "$(tail -n 1 "$SERIES_FILE")" == \
  "0136-feat-aegis-terminate-inflight-block-requests.patch" ]] ||
  fail "patch 0136 must be the current series tail"
