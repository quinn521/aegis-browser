...
SERVICE_WORKER_SCRIPT_PATCH_FILE="$BROWSER_DIR/patches/0146-feat-aegis-gate-service-worker-script-traffic.patch"
...
rg -Fq 'URLLoaderFactoryType::kServiceWorkerScript && !frame' \
  ...
  fail "patch 0146 must wire frame-less ServiceWorker script factories"
rg -Fq 'render_process_id != content::ChildProcessHost::kInvalidUniqueID' \
  "$SERVICE_WORKER_SCRIPT_PATCH_FILE" ||
  fail "patch 0146 must leave browser-process ServiceWorker script/update fetches native"
rg -Fq 'MaybeProxyServiceWorkerScript' "$SERVICE_WORKER_SCRIPT_PATCH_FILE" ||
  fail "patch 0146 must install the process-backed ServiceWorker script wrapper"
rg -Fq 'MaybeProxyProfileOnlyFactory(profile, render_process_id, factory_builder)' \
  "$SERVICE_WORKER_SCRIPT_PATCH_FILE" ||
  fail "patch 0146 must reuse trusted Profile-only process ownership"
rg -Fq 'ServiceWorkerScriptWithoutPolicyPreservesNativePath' \
  "$SERVICE_WORKER_SCRIPT_PATCH_FILE" ||
  fail "patch 0146 must preserve native ServiceWorker script traffic without policy"
rg -Fq 'ServiceWorkerImportedScriptUsesSelectedProxy' \
  "$SERVICE_WORKER_SCRIPT_PATCH_FILE" ||
  fail "patch 0146 must browser-test selected proxy imported-script routing"
rg -Fq 'ServiceWorkerImportedScriptWithoutEndpointFailsClosed' \
  "$SERVICE_WORKER_SCRIPT_PATCH_FILE" ||
  fail "patch 0146 must fail closed when imported-script PROXY lacks an endpoint"
if rg -Fq 'URLLoaderFactoryType::kPrefetch' "$SERVICE_WORKER_SCRIPT_PATCH_FILE"; then
  fail "patch 0146 must not claim prefetch coverage"
fi
...
[[ "$(rg -F -c '0146-feat-aegis-gate-service-worker-script-traffic.patch' \
...
0146-feat-aegis-gate-service-worker-script-traffic.patch
EOF
)"
