...
void MaybeProxyFrame(
    Profile* profile,
    content::RenderFrameHost* frame,
    std::optional<int64_t> navigation_id,
    bool require_primary_main_frame,
    network::URLLoaderFactoryBuilder& factory_builder) {
  DCHECK_CURRENTLY_ON(content::BrowserThread::UI);
  ...
  if (!contents || contents->GetBrowserContext() != profile ||
      (require_primary_main_frame &&
       contents->GetPrimaryMainFrame() != frame)) {
    return;
  }
...
void AccessProxyingURLLoaderFactory::MaybeProxyDocumentSubresource(
    ...
    content::RenderFrameHost* frame,
    network::URLLoaderFactoryBuilder& factory_builder) {
  MaybeProxyFrame(profile, frame, std::nullopt,
                  /*require_primary_main_frame=*/false, factory_builder);
}

// static
void AccessProxyingURLLoaderFactory::MaybeProxyPrimaryNavigation(
    Profile* profile,
    content::RenderFrameHost* frame,
    int64_t navigation_id,
    network::URLLoaderFactoryBuilder& factory_builder) {
  MaybeProxyFrame(profile, frame, navigation_id,
                  /*require_primary_main_frame=*/true, factory_builder);
}
