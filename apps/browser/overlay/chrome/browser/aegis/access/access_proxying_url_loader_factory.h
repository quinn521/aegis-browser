...
// UI-thread browser-process URLLoaderFactory wrapper for real Access requests.
// It covers primary-page document subresources plus primary main-frame
// navigation. Nested-frame navigation, workers, WebSocket, and redirect
// re-evaluation are later slices. Each request is re-evaluated from
...
class AccessProxyingURLLoaderFactory : public network::mojom::URLLoaderFactory {
 public:
  ...
  static void MaybeProxyDocumentSubresource(
      ...
      network::URLLoaderFactoryBuilder& factory_builder);
  static void MaybeProxyPrimaryNavigation(
      Profile* profile,
      content::RenderFrameHost* frame,
