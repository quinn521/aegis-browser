...
AccessNetworkContextTransport::RepublishCurrentConfigWithAck(
  ...
  auto fail_publication =
      [&](AccessNetworkConfigAckStatus status) -> AccessNetworkConfigAckResult {
    std::move(all_clients_settled).Run(false);
    return {status, 0};
  };
  if (!OwnsConfiguredPartition(owner)) {
    return fail_publication(AccessNetworkConfigAckStatus::kInvalidOwner);
  }
  auto it = partitions_.find(owner.storage_partition_token);
  if (it == partitions_.end()) {
    return fail_publication(AccessNetworkConfigAckStatus::kMissingPartition);
  }
  PartitionState& state = it->second;
  if (state.clients.empty()) {
    return fail_publication(AccessNetworkConfigAckStatus::kNoClients);
  }
